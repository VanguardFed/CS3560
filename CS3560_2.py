...
def translate(text):
    ...
    #for each letter i, if i is a consonant insert i+o+i
    for i in text:
...
def multiply(num_list):
    ...
    if 0 in num_list or len(num_list)==0:
        return 0
    #otherwise, multiply all num together
    for num in num_list:
        total=total*num
    return total
...
def reverse(text):
    ...
    for i in range (len(text)):
        j=-1*(i-len(text))-1
        #starting from the end of 'text' going towards the front, add letter j to 'reversed'
        reversed+=text[j]
    return reversed

def is_palindrome(text):
    #if it is the same when reversed, it is a palindrome
    return True if text==reverse(text) else False

def overlapping(list1, list2):
    for i in range(0,len(list1)):
        if list1[i]==list2[i]:
            #if any element is the same in the same location, return True
            return True
    return False

def histogram(num_list):
    for i in range (len(num_list)):
        #print stars number by number
        print '*'*num_list[i]
    return None
...
def find_longest_word(word_list):
    ...
    for i in range(0,len(word_list)):
        if len(word_list[i])>len(longest):
            #if current word is larger than 'longest', change it to 'longest'
            longest=word_list[i]
    return longest
...
def filter_long_words(word_list,len_limit):
    ...
    for i in range(0,len(word_list)):
        if len(word_list[i])>len_limit:
            #if word is smaller than 'limit' add it to 'long_list'
            long_list.append(word_list[i])
    return long_list

def is_palindrome_sentence(phrase):
    #if it is a palindrome when all punctuation and uppercases are removed, return True
    return is_palindrome(filter(str.isalpha,phrase.lower()))

def is_pangram(phrase):
    all_letters=['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z']
    for i in range(0,len(all_letters)):
        #if it does not have every letter in list 'all_letters', return False
